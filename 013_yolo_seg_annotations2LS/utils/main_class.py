...
class YoloSeg2LabelStudio:
    ...
    def get_labels(self, path_labels, path_images, labelst_part_path):
        ...
        res = Result()
        # list_main_part = []
        image_dict = {os.path.splitext(os.path.basename(img))[0]: img for img in self.list_images}
    
        list_main_part = []
        
        for label_path in self.list_labels:
            label_base = os.path.splitext(os.path.basename(label_path))[0]
            if label_base in image_dict:
                path_img = image_dict[label_base]
                ...
                print(f"Image {label_base} not found in {path_images}")
